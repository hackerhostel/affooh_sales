...
export const onToolbarPreparing = (e) => {
  ...
  const columnChooserButton = toolbarItems.find(
    (item) => item.name === "columnChooserButton"
  );
...
export const customCellRender = (data) => {
  if (typeof data.value === "object") {
    return (
      <div className="text-sm text-center">{formatDateIfDate(data.value)}</div>
    );
  }
  return <div className="text-sm text-center">{data.value}</div>; 
};

export const customHeaderRender = (data) => {
  return (
    <div className="font-bold text-gray-600 text-center ml-4">
      {data.column.caption}
    </div>
  ); 
};
...
export const statusCellRender = (data) => {
  ...
  return (
    ...
    >
      <div
        className={`${bgBoldColors[status] || "bg-secondary-bgc"} min-w-1 rounded-md`}
      ></div>
...
export const relationCellRender = (data) => {
  ...
  return (
    ...
    >
      <div
        className={`${bgBoldColors[type] || "bg-secondary-bgc"} min-w-1 rounded-md`}
      ></div>
      {type}
    </div>
  );
...
export const assigneeCellRender = (data) => {
  return (
    <div className="text-sm text-wrap text-center flex items-center gap-1 cursor-pointer">
      {" "}
      
      <div className=" object-center  w-8 h-8 rounded-full bg-primary-pink flex items-center justify-center text-white text-sm font-semibold">
...
export const removeObjectFromArrayByDataField = (arr, dataFieldToRemove) => {
  ...
  const removedIndex = arr.findIndex(
    (obj) => obj.dataField === dataFieldToRemove
  );
